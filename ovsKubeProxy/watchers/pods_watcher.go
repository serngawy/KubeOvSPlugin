...
func (pw *podWatcher) podAddEventHandler(obj interface{}) {
	...
	pw.broadcaster.Notify(&PodUpdate{Op: utils.ADD, Pod: pod})
}
...
func (pw *podWatcher) podDeleteEventHandler(obj interface{}) {
	...
	pw.broadcaster.Notify(&PodUpdate{Op: utils.REMOVE, Pod: pod})
}

func (pw *podWatcher) podUpdateEventHandler(oldObj, newObj interface{}) {
	pod, ok := newObj.(*api.Pod)
	if !ok {
		return
	}
	if !reflect.DeepEqual(newObj, oldObj) {
		pw.broadcaster.Notify(&PodUpdate{Op: utils.UPDATE, Pod: pod})
	}
}
...
func StartPodWatcher(clientset *kubernetes.Clientset, resyncPeriod time.Duration) (*podWatcher, error) {
	...
	eventHandler := cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: pw.podUpdateEventHandler,
	}
